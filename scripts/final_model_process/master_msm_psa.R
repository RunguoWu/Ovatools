...
source(file.path(r_wd, "scripts/HE_model/fn_msm_stpm2.R"))
...
for (s in 8:8) {
  ...
  qol_list <- readRDS(file.path(work_data, "psa", "qol_list.rds"))
  # cancer death probability PSA list 
  surv_prob <- if (c_type == "benign") NULL else 
    surv_prob <- readRDS(file.path(work_data, "psa", paste0("surv_prob_", c_type, "_all.rds")))
  
  # cost model coefficient PSA file
  cost_coef <- readRDS(file.path(work_data, "psa", paste0("cost_coef_int_psa_all.rds")))
  ...
  for (num in 237:n_sim) {
    ...
    # output
    output_folder <- file.path(r_wd, "output", "psa", paste0(c_type, "_", stage_late))
