...
fm_vec <- c(ova = as.formula("cancer_ova_1yr ~ age_group_40_80 + ethn4 + town5"), 
            lung = as.formula("cancer_lung_1yr ~ age_group_60_80 + ethn4 + town5"), 
            loGI = as.formula("cancer_loGI_1yr ~ age_group_50_80 + ethn4 + town5"), 
            uter= as.formula("cancer_uter_1yr ~ age_group_50_80 + ethn4 + town5"), 
            panc = as.formula("cancer_panc_1yr ~ age_group_60_80 + ethn4 + town5"), 
...
for (cancer_type in c_list) {
  ...
  set.seed(1234)
  boots_ids <- replicate(N, sample(1:nrow(dt), nrow(dt), replace = TRUE))
  ...
  pred <- foreach (i = 1:N) %dopar%{
    ...
    pred <- predict(fit, dt, type = "response")
  ...
  if(cancer_type %in% c("loGI", "panc", "uter", "lung")) {
    dt2 <- dt[dt[[paste0("b_", cancer_type)]]==0, ]
  } else if(cancer_type =="other" ) {
    dt2 <- dt[dt[["b_cancer_others"]]==0, ]
  } else {
    dt2 <- dt}
  
  fit <- glm(fm, dt2, family="binomial")
  pred_det <-  predict(fit, dt, type = "response")
  pred[[length(pred) + 1]] <- pred_det
  saveRDS(pred, file.path(work_data, "psa/prev", paste0(cancer_type, "_prev.rds")))
} 
...
for (cancer_type in c("ova", "loGI", "panc", "uter", "lung", "other")) {
  ...
  fit <- glm(fm, dt2, family="binomial")
  write.csv(print_glm_model(fit), file.path(output, paste0(cancer_type, "_prevMod.csv")))
}
...
fm_vec <- c(ova = as.formula("stage_late ~ age_group_40_80 + ethn4 + town5"), 
            ...
            loGI = as.formula("stage_late ~ age_group_50_80 + ethn4 + town5"), 
            uter= as.formula("stage_late ~ age_group_50_80 + ethn4 + town5"), 
            panc = as.formula("stage_late ~ age_group_60_80 + ethn4 + town5"), 
...
for (cancer_type in c_list) {
  ...
  for (imp in 1:length(imp_list)) {
    ...
    pred_imp <- cbind(pred_imp, predict(fit, type = "response", dt))
  }
  pred_avg <- rowMeans(pred_imp)
  pred[[length(pred) + 1]] <- pred_avg
  
  saveRDS(pred, file.path(work_data, "psa/stage_rate", paste0(cancer_type, "_stage.rds")))
...
for (cancer_type in c("ova", "loGI", "panc", "uter", "lung", "other")) {
  ...
  write.csv(coef, file.path(output, paste0(cancer_type, "_stageMod.csv")))
}
...
prev <- readRDS(file.path(wd_prev, "ova_prev.rds"))
stage <- readRDS(file.path(wd_stage, "ova_stage.rds"))
...
foreach(i = 1:length(prev))%dopar%{
  ...
  saveRDS(x, file.path(wd_prev, "ova", paste0("ova_prev", i, ".rds")))

  y <- data.frame(ova_stage34 = stage[[i]])
  saveRDS(y, file.path(wd_stage, "ova", paste0("ova_stage", i, ".rds")))
...
for (c_type in c("lung", "panc", "uter", "other")) {
  
  if(c_type=="other") base <- "b_cancer_others" else
    base <- paste0("b_", c_type)
  
  dtt <- dt %>% select(!!sym(base))
  stage <- readRDS(file.path(wd_stage, paste0(c_type, "_stage.rds")))
  prev <- readRDS(file.path(wd_prev, paste0(c_type, "_prev.rds")))
  ...
  foreach(i = 1:length(stage))%dopar%{
    ...
    saveRDS(rt, file.path(wd_prev, c_type, paste0(c_type, "_prev", i, ".rds")))
    saveRDS(rt2, file.path(wd_stage, c_type, paste0(c_type, "_stage", i, ".rds")))
